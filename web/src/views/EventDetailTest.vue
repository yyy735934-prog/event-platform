  ...
  <div class="page event-test">
    ...
    <template v-else>
      <header class="event-hero">
        <router-link to="/" class="hero-back">← 活动列表</router-link>
        <p>活动详情 · 测试版</p>
        <h1>{{ event.title }}</h1>
        <div>{{ event.event_date }}<span v-if="event.location"> · {{ event.location }}</span></div>
        <div class="hero-bottom"><span>{{ chatToken ? '✓ 已报名' : managerSession ? '主办方' : { open:'报名中', active:'进行中', closed:'已结束' }[event.status] || event.status }}</span><span>{{ event.signupCount }} 人参加</span></div>
      </header>
      ...
      <div v-if="event.notes" class="card mb test-notice"><strong>📌 最新通知 · 主办方</strong><p>{{ event.notes }}</p></div>
      <div v-if="chatAvailable" class="card mb summary-card">
        <div class="summary-heading"><strong>💬 活动讨论</strong><span v-if="unread">{{ unread }} 条未读</span></div>
        <div v-for="item in summaries" :key="item.id" class="summary-item">
          <strong>{{ item.sender }}</strong><p>{{ item.text }}</p>
          <small v-if="item.replyCount">↳ {{ item.organizerReplied ? '主办方已回复 ✓ · ' : '' }}{{ item.replyCount }} 条回复<span v-if="item.unreadReplies"> · {{ item.unreadReplies }} 条新回复</span></small>
          <small v-if="item.latestSender">最新：{{ item.latestSender }}：{{ item.latestText }}</small>
        </div>
        <p v-if="!summaries.length">暂无活动讨论</p>
        <button type="button" class="btn btn-primary" @click="selectTab('discussion')">进入活动讨论</button>
      </div>
      <div v-else class="card mb summary-card"><strong>💬 活动讨论</strong><p>报名后开放</p></div>
      <!-- Event Info -->
      <div class="card mb">
      ...
      <section v-show="activeTab === 'discussion'" class="tab-panel">
        ...
        <div v-else class="card locked-tab"><h2>💬 活动讨论</h2><p>报名后自动加入本活动讨论。这里用于集合、装备、同行、临时安排和活动照片交流。</p><button type="button" class="btn btn-outline" @click="selectTab('detail')">报名后开放</button></div>
      </section>
      <section v-show="activeTab === 'people'" class="tab-panel card people-panel">
        <h2>已确认参加 {{ event.signupCount }} {{ effectiveCap ? `/ ${effectiveCap}` : '' }} 位</h2>
        <template v-if="chatAvailable"><div v-for="person in people" :key="person.uid" class="person"><span>{{ person.name }}</span><strong>{{ ['moderator','admin','owner'].includes(person.scope) ? '主办方' : '活动参与者' }}</strong></div></template>
...
function onDiscussionUpdate(data) {
  if (data.summaries) summaries.value = data.summaries
  if (data.people) people.value = data.people
  if (data.unread !== undefined) unread.value = data.unread
}
function selectTab(tab) {
  activeTab.value = tab
  router.replace({ path: route.path, query: tab === 'detail' ? {} : { tab } })
  if (tab === 'detail') discussionRef.value?.refresh()
  if (tab === 'discussion') setTimeout(() => discussionRef.value?.refresh(), 700)
}
...
onMounted(async () => {
  ...
  if (event.value && auth.isLoggedIn && !chatToken.value) {
    try { managerSession.value = await api.chatSession({ event_id: Number(route.params.id) }) } catch { /* Only permitted managers receive a session. */ }
  }
